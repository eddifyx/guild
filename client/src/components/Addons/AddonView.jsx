...
export default function AddonView() {
  ...
  const handleUpload = useCallback(async (file) => {
    ...
    setUploadError(null);
    try {
      await uploadAddon(file, description.trim() || null, (pct) => setUploadProgress(pct));
      setDescription('');
      ...
      >
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
          ...
          >
            ...
            {uploading ? 'Uploading...' : 'Upload Addon'}
          </button>
        </div>
        <div
        ...
        {uploadError && (
          <div style={{
            ...
            <button onClick={() => setUploadError(null)}
              style={{ background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', padding: 0, fontSize: 14, lineHeight: 1 }}>x</button>
          </div>
        )}
      </div>
      ...
      >
        {loading ? (
          ...
          <div style={{
            ...
            {addons.map(addon => {
              const isOwner = addon.uploaded_by === user.userId;
              const url = getFileUrl(addon.file_url);
              const isImage = (addon.file_type || '').startsWith('image/');
                ...
                >
                  ...
                  <div style={{ padding: '12px 14px' }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button
                        onClick={() => handleDownload(addon.file_name, url)}
                        style={{
