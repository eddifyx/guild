...
test('publishProfile preflights nip46 signers and retries once after a stale signer timeout', async () => {
  const prepareCalls = [];
  const signCalls = [];
  const eventPubkeys = [];
  ...
  assert.deepEqual(signCalls, ['stale-signer', 'fresh-signer']);
  assert.deepEqual(eventPubkeys, ['pubkey-1', 'pubkey-1']);
