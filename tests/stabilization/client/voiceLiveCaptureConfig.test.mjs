...
test('voice live capture config forces the plain reference lane during the controlled reset', () => {
  const config = buildVoiceLiveCaptureConfig({
    mode: 'balanced',
    ...
    buildVoiceCaptureConstraintsFn: ({ mode, noiseSuppressionEnabled }) => ({
      audio: { mode, noiseSuppressionEnabled },
    }),
    isUltraLowLatencyModeFn: (mode) => mode === 'ultra',
    ultraLowLatencyMode: 'ultra',
    webrtcApmBackend: 'webrtc-apm',
  ...
  assert.deepEqual(config, {
    activeVoiceProcessingMode: 'balanced',
    referenceVoiceLane: true,
    captureConstraintMode: 'ultra',
    useRawMicPath: true,
    noiseSuppressionEnabled: false,
    inputId: '',
    requestedInputId: '',
    preferSystemMicMode: false,
    preferAppleVoiceProcessing: false,
    requestedSuppressionRuntime: {
      backend: 'raw',
      requiresWarmup: false,
