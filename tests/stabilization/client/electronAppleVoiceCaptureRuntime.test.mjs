...
} = require('../../../client/electron/appleVoiceCaptureRuntime.js');
...
test('electron apple voice capture runtime starts, forwards ready state, and resolves on first frame', async () => {
  ...
  const runtime = createAppleVoiceCaptureRuntime({
    ...
    sendState: (payload) => states.push(payload),
...
test('electron apple voice capture runtime disables the feature on fatal helper errors', async () => {
  const proc = createProc();
  const states = [];
  const state = {
    disabledReason: null,
