...
test('audio settings model can force a truly plain mic-test capture request during the reference reset lane', () => {
  const defaultConstraints = buildMicTestConstraints({
    deviceId: '',
    plain: true,
  });
  const selectedDeviceConstraints = buildMicTestConstraints({
    deviceId: 'mic-1',
    plain: true,
  });

  assert.equal(defaultConstraints.audio, true);
  assert.deepEqual(selectedDeviceConstraints.audio, {
