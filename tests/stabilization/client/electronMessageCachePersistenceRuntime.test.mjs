...
function createTmpUserDataDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'guild-electron-message-cache-persist-'));
}
...
test('electron message cache persistence runtime upgrades legacy cache files and prunes invalid or stale entries', () => {
  ...
  const runtime = createMessageCachePersistenceRuntime({
    app: { getPath: () => userDataDir },
    fs,
    path,
...
test('electron message cache persistence runtime removes persisted files when protected storage is unavailable and schedules flush once', () => {
