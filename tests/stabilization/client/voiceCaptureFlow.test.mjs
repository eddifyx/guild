...
test('voice capture flow builds the plain reference capture lane with a gain-backed destination track', async () => {
  const sourceTrack = {
    id: 'audio-track-reference',
    kind: 'audio',
    enabled: false,
    readyState: 'live',
    getSettings() {
  ...
  class FakeAudioContext {
    ...
    createMediaStreamDestination() {
      return {
        stream: {
          getAudioTracks() {
            return [{ id: 'processed-track-reference', enabled: false }];
          },
        },
      };
    }
  ...
  const result = await createVoiceLiveMicCapture({
    ...
    deps: {
      ...
      audioContextCtor: FakeAudioContext,
      performanceNowFn: () => 40,
      nowIsoFn: () => '2026-03-31T21:00:00.000Z',
      roundMsFn: (value) => value,
      summarizeTrackSnapshotFn: (value) => value?.id || null,
      warnFn: () => {},
    },
  ...
  assert.ok(result.capture);
  assert.equal(result.capture.outputTrack.id, 'audio-track-reference');
  assert.equal(result.capture.outputTrackMode, 'direct-source-hotfix');
  assert.equal(result.diagnostics.filter.backend, 'raw');
  assert.equal(result.diagnostics.filter.fallbackReason, 'Reference voice lane active');
  assert.equal(result.diagnostics.producedTrack, 'audio-track-reference');
});
