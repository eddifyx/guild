...
test('voice output device helpers apply sink ids with safe fallback behavior', async () => {
  ...
  const defaultSinkCalls = [];
  const defaultAudio = {
    async setSinkId(deviceId) {
      defaultSinkCalls.push(deviceId);
    },
  };
  const defaultAppliedDevice = await applyVoiceOutputDevice(defaultAudio, 'default');
  assert.equal(defaultAppliedDevice, 'default');
  assert.deepEqual(defaultSinkCalls, []);
  ...
  const defaultBulkCalls = [];
  const defaultAudioA = { setSinkId: async (deviceId) => { defaultBulkCalls.push(['a', deviceId]); } };
  const defaultAudioB = { setSinkId: async (deviceId) => { defaultBulkCalls.push(['b', deviceId]); } };
  assert.equal(applyVoiceOutputDeviceToAll([defaultAudioA, defaultAudioB], 'default'), 'default');
  await Promise.resolve();
  assert.deepEqual(defaultBulkCalls, []);
});
