...
test('audio settings monitor preview runtime creates the canonical preview audio element and stores its ref', () => {
  ...
  assert.equal(previewAudio.defaultMuted, false);
  assert.equal(previewAudio.position, 'absolute');
  assert.equal(previewAudio.width, '1px');
  assert.equal(previewAudio.height, '1px');
  assert.equal(previewAudio.opacity, '0.001');
