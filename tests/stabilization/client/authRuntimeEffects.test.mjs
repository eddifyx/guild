...
test('runNip46SessionKeepalive is a no-op for non-nip46 sessions', async () => {
  const result = await runNip46SessionKeepalive({
    getLoginModeFn: () => 'nsec',
  });
...
test('runNip46SessionKeepalive keeps a live nip46 signer warm with ping only', async () => {
  const calls = [];
  const result = await runNip46SessionKeepalive({
    ...
    waitForNip46RelayCooldownFn: async (stage) => {
      calls.push(stage);
    },
  });

  assert.deepEqual(calls, [
    'before_nip46_keepalive_ping',
    'ping',
  ]);
  assert.deepEqual(result, {
    attempted: true,
...
test('runNip46SessionKeepalive reconnects on foreground when the signer is stale', async () => {
  const calls = [];
  let signer = {
    ping: async () => {
      calls.push('stale-ping');
  ...
  assert.deepEqual(calls, [
    ...
    'stale-ping',
    'reconnect',
    'after_nip46_keepalive_reconnect',
    'fresh-ping',
  ]);
  assert.deepEqual(result, {
    attempted: true,
...
test('registerNip46SessionKeepalive wires interval and foreground refresh for signed-in users', async () => {
  const listeners = new Map();
  const documentListeners = new Map();
  const keepaliveCalls = [];
  let intervalHandler = null;

  const cleanup = registerNip46SessionKeepalive({
    user: {
      userId: 'user-1',
      npub: 'npub1builder',
      token: 'token-1',
    ...
    keepaliveFn: async ({ reconnectOnFailure }) => {
      keepaliveCalls.push(reconnectOnFailure);
    },
